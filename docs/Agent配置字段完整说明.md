...
本服务对 `config` 不做字段级校验，作为 JSON 存储并交由 `maim_db.core.AgentConfigManager` 读取/写入。以下为推荐结构，便于与 MaiMBot 运行时对齐。

## 顶层字段
- `tenant_id` (必填)：租户 ID（由调用者提供）。
- `name` (必填)：Agent 名称（同租户内唯一）。
- `description` (可选)：描述。
- `template_id` (可选)：模板引用。
- `config` (可选，JSON)：自由结构；推荐使用下列分段。
- `tags` (可选，数组)：标签列表（当前代码未自动保存，可放入 config）。

## 推荐的 config 分段
- `persona`：人格/风格描述，例如 `personality`、`reply_style`、`interest`、`states`、`state_probability`。
- `bot_overrides`：Bot 基础信息，如 `platform`、`qq_account`、`nickname`、`alias_names`。
- `config_overrides`：运行时配置覆盖，常见子项：
  - `chat`: `max_context_size`, `planner_size`, `auto_chat_value`, `talk_value`, `interest_rate_mode` ...
  - `memory`: `max_memory_number`, `memory_build_frequency`。
  - `mood`: `enable_mood`, `mood_update_threshold`, `emotion_style`。
  - `emoji`: `emoji_chance`, `max_reg_num`, `steal_emoji`, `content_filtration`。
  - `tool`: `enable_tool`。
  - `expression`: `mode`, `learning_list`, `expression_groups`。
  - `keyword_reaction`: `keyword_rules`, `regex_rules`。
  - `relationship`: `enable_relationship`。
...
## 约束与建议
- 名称在同租户内应唯一，服务端会做重复校验。
- `config` 建议保持 JSON 对象，避免在根层放置非对象类型。
- 权重/概率类字段保持 0~1 区间；大小/计数类字段保持正数，以免下游出错。
- Agent 创建后服务会自动调用活跃心跳上报（TTL 默认 12h），与配置无关。
